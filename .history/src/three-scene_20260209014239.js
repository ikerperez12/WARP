...
function createLaptopModel(textures) {
  ...
  deckBevelLeft.position.set(-1.56, 0.088, 0.13);
  ...
  const fanRingGeo = useGeometry(new THREE.TorusGeometry(0.245, 0.008, 8, 44));
  const fanLeft = new THREE.Mesh(fanRingGeo, detailMat);
  fanLeft.rotation.x = Math.PI * 0.5;
  fanLeft.position.set(-0.66, 0.102, -0.66);
  root.add(fanLeft);
  const fanRight = fanLeft.clone();
  fanRight.position.x = 0.66;
  root.add(fanRight);

  const fanBladeGeo = useGeometry(new THREE.BoxGeometry(0.004, 0.002, 0.16));
  const fanBlades = new THREE.InstancedMesh(fanBladeGeo, portMat, 24);
  for (let i = 0; i < 24; i++) {
    const side = i < 12 ? -1 : 1;
    const idx = i % 12;
    const angle = (idx / 12) * Math.PI * 2;
    tmp.position.set(side * 0.66 + Math.cos(angle) * 0.13, 0.102, -0.66 + Math.sin(angle) * 0.13);
    tmp.rotation.set(Math.PI * 0.5, angle, 0);
    tmp.updateMatrix();
    fanBlades.setMatrixAt(i, tmp.matrix);
  }
...
export function initThreeScene() {
  ...
  camera.position.set(0.18, 0.64, 6.95);
  ...
  const baseX = isMobile ? 0.56 : 2.98;
  ...
  function onScroll() {
    ...
    const heroRange = heroSection ? Math.max(heroSection.offsetHeight * 1.05, window.innerHeight * 1.05) : Math.max(window.innerHeight * 1.1, 720);
    const heroProgress = clamp01((window.scrollY - heroStart) / heroRange);
    const viewportKick = clamp01(window.scrollY / Math.max(window.innerHeight * 0.85, 520));

    state.scrollTarget = clamp01(Math.max(globalProgress * 0.42, heroProgress, viewportKick * 0.78));
  }
  ...
  function render() {
    ...
    const basePitch = -0.008 + Math.sin(t * 0.58) * 0.009 * motionFactor;
    ...
    const targetPitch = basePitch - pointerCurveY * 0.15 * motionFactor - velocityKick * 0.3;
    ...
    const targetY = 0.13 + bob - pointerCurveY * 0.078 * motionFactor - state.scroll * 0.25 - velocityKick * 0.06;
    ...
    const openAngle = -0.43 - state.scroll * 0.055 + Math.sin(t * 0.3) * 0.009 * motionFactor + velocityKick * 0.05;
    ...
    const carrierTargetX = isMobile ? -1.2 : -2.18;
    const carrierTargetY = -0.078 - state.scroll * 0.01;
    const carrierTargetZ = isMobile ? 0.34 : 0.44;
    ...
    const padScale = 0.001 + mousePadReveal * 1.2;
    ...
    const mouseLocalX = (isMobile ? -0.02 : -0.12) + (1 - mouseReveal) * -0.08;
    const mouseLocalY = 0.052;
    const mouseLocalZ = 0.004;
    ...
    const mouseScale = 0.001 + mouseReveal * 0.68;
    ...
    const camTargetY = (0.42 - pointerCurveY * 0.14 - state.scroll * 0.09) * motionFactor + (1 - motionFactor) * 0.14;
    const camTargetZ = 6.95 + state.scroll * 0.24;
    ...
    camera.lookAt(baseX * 0.38, 0.2 - state.scroll * 0.08, -0.1);
